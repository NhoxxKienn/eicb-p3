...
public class InteractiveInterpreter extends Interpreter {
	...
	private void executeLoadCmd(String path) throws ExecutionException {
		if(path == null) {
		...
		basePath = Paths.get(path).getParent();
		
		Image image;
		try {
			if(path.endsWith(".mavl")) {
				Pipeline pipeline = new Pipeline();
				pipeline.parseProgram(Paths.get(path));
				pipeline.throwError();
				...
				image = new Image(Paths.get(path));
				image.load();
				state.throwIfError();
				
				Path symPath = Paths.get(path.replace(".tam", ".sym"));
		...
		if(lineNumber >= 0 && lineNumber < disassembly.length) {
			...
			switch(line.type) {
				...
				case instruction:
					...
					output.print(String.format("%04x: ", line.address));
					// instruction
					if(isActive) {
	...
	private void printStackLine(int displayLine) {
		...
		if(overflow > 0 && displayLine == 0) {
			...
			output.print(String.format(" %04x: ", stackAddress));
			// contents
	...
	private void printValue(Value value, int width, boolean isActive) {
		...
		switch(value.type) {
			...
			case boolT:
				...
				break;
			case floatT:
				output.print(padLeft(String.valueOf(value.asFloat()) + "f", width));
				break;
			case cAddrT:
